...
It selects the newest valid snapshot when opened and checks again after 60 seconds without input. Turn the wheel to browse at most 20 events, press OK for details, and press Back to return. The front uses two slowly scrolling lines beside the event icon: event name above, date and start time below. Both lines share one synchronized marquee so they move together. Cached data older than one hour is labeled stale.
